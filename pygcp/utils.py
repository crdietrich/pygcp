...
def has_keys(d, keys):
    ...
    ----------
    d : dictionary
    keys : str or list of str, dictionary key(s) to check for

    Returns
    -------
    bool, True if all keys are in the dictionary
    """
    if pd.isnull(d) or not isinstance(d, dict):
        return False
...
def has_values(d, values):
    ...
    ----------
    d : dictionary
    values : str or list of str, dictionary value(s) to check for
    ...
    if isinstance(values, str):
        values = [values]
    return all(v in d.values() for v in values)
...
def has_keys_and_values(d, kv):
    ...
    """
    if (pd.isnull(d)) or (not isinstance(d, dict)) or (not isinstance(kv, dict)):
        return False
