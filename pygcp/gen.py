...
class GenAIBase:
    """Base Class for Generative AI.

    Attributes
    ----------
    project_id : str
        GCP project ID to host model prediction
    location_id : str
        GCP location to run the model prediction
    """
    ...
    def vertex_init(self, project_id, location_id):
        """TODO: Docstring for vertex_init."""
...
class GenAITextSummary(GenAIBase):
    ...
    ----------
    temperature : float
        Randomness of predictions. Range of 0.0 and 1.0
    max_output : int
        maximum amount of text output in tokens
    top_p : float
        Cumulative probability of highest probability vocabulary tokens
        to keep for nucleus sampling. Range of 0.0 to 1.0.
    top_k : int
